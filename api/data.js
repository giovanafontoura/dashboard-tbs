...
// Retry automático: 429 → 2s wait; 5xx → backoff; 4xx fixo → retorna sem retry
...
// Qualifica contato pelo fonte ou pelo utm_term
...
async function fetchAllContacts(token) {
  ...
  // Filtros em ordem de preferência — fonte/data não dependem de inscrito_tbs_2026
  const FILTER_SETS = [
    // 1. Filtro original (propriedade pode ter mudado — tenta mesmo assim)
    [{ filterGroups: [{ filters: [{ propertyName: 'inscrito_tbs_2026', operator: 'EQ', value: 'Sim' }] }] }],
    // 2. Direto pela fonte: WA (organic social + whatsapp) OU Email Marketing com data TBS 2026
    [{
      filterGroups: [
        { filters: [
          { propertyName: 'fonte__tbs_', operator: 'CONTAINS_TOKEN', value: 'whatsapp' },
    ...
    // 3. Só pela data de inscrição TBS 2026 (amplo — isWA/isEM filtra depois)
    [{ filterGroups: [{ filters: [{ propertyName: 'tbs_2026__data_de_inscricao', operator: 'IS_KNOWN' }] }] }],
  ];

  for (const [filterBody] of FILTER_SETS) {
    let result;
    try {
      result = await _paginate(token, ENDPOINT, HEADERS, filterBody);
    } catch (e) {
      if (e.message.includes(' 400')) {
        console.warn(`[contacts] 400 → próximo filtro`);
        continue;
      }
      throw e;
    }
    if (result.length === 0) {
      console.warn(`[contacts] 0 contatos → próximo filtro`);
      continue;
    }
    // Verifica se o filtro retornou contatos WA/EM reais
    const qualified = result.filter(({ properties: p }) => qualifyContact(p));
    if (qualified.length === 0 && result.length > 0) {
      console.warn(`[contacts] ${result.length} contatos mas nenhum qualificado → próximo filtro`);
      continue;
    }
    console.log(`[contacts] OK → ${result.length} total, ${qualified.length} qualificados`);
    return result;
  }
  throw new Error('Nenhum filtro de contatos retornou dados qualificados');
}

async function _paginate(token, endpoint, headers, filterBody) {
  const all = [];
  let after;
  do {
...
async function fetchDealsWithTerms(token) {
  ...
  // 1. Todos os deals na etapa Negócio Fechado / The Best School
  ...
  // 3. Propriedades dos contatos associados
  const contactProps = {};
